...
function ( geno, des=0, lim=0.05 )
{
  ...
  if ( (!is.numeric(lim)) || (lim<0.0) || (lim>1.0) ) stop ("Bad value for lim.")
  ...
  # max 16 SNPs
  ns <- dim(geno)[2]
  if ( ns > 16 ) stop ( "Number of SNPs should smaller than 17."  )
  ...
  # call C function
  zzz <- .C( "itegeppXXR", as.integer(des),as.double(lim) ,
             as.character(genotyp), as.double(1),
             as.integer(len), likeres = as.double(0.1,0) , freqres = as.character(fr),
             hapres = as.character(hr), desres = as.character(dr) , PACKAGE="HapEstXXR" )
  
  hr <- zzz$hapres[ zzz$hapres!=lest ]

  zzz$freqres <- as.matrix(zzz$freqres[zzz$freqres != lest])

  hp <- as.matrix(apply(zzz$freqres,1,function(x) unlist(strsplit(x,split=" "))))

  if ( dim(hp)[1]==0 )
  {
    print(paste ( "Error in itegeppXXR: all inferred haplotypes with probability " ,
  ...
  zzz$desres <- t ( apply( (as.matrix(zzz$desres)) , 1,
     fsplit ) )
