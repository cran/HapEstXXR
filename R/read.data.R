...
{
    if (linkage == TRUE) {
        f <- unlist(strsplit(readLines(filename, n = 1), split = "[ \t\n\f\r\v]+"))
        f <- f[f != ""]
        lf <- length(f)
        nmarker <- (lf - 6)
        if ((nmarker%%2) != 0)
            stop("Line 1 did not have expected structure.")
        tab <- read.table(filename,  colClasses = c(rep("character",
            5), "numeric", rep("character", nmarker)))
    ...
    } else  #(linkage == FALSE)  ##=> Klaus format
    {
        ...
        if (lf == 3) {
            tab <- read.table(filename,  colClasses = c("character",
                "character", "numeric"))
            if (ncol(tab) != 3)
                stop("incorrect file.")
            patid <- tab[, 1]
            geno <- matrix(as.numeric(unlist(strsplit(tab[, 2],
                split = ""))), nrow = length(patid), byrow = TRUE)
            pheno <- tab[, 3]
            famid <- patid
            nind <- length(pheno)
            fid <- rep(0, nind)
    ...
    chr  <- pos <- snp <- NULL
    if (!is.na(map)) {
        inmap <- read.table(file = map, colClasses = c("numeric",
    ...
    rownames(geno) <- patid
    colnames(geno) <- snp
    return(list(famid = famid, patid = patid, fid = fid, mid = mid,
