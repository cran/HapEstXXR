...
if ( (j%%5000)==0 ) { cat(paste("Step =  ",j,"   System.time = ", (Sys.time()), "\n",sep="")) }
          ...
          hap.test <- msr.gaussian.haplotype.test.unadjusted ( geno.pair, trait , lim =lim,
    ...
    #################################################
    # Next iteration: from 2,3,4 ... (or colnumber of pattern.begin.mat) to maxSNP
    ...
    while ( i <= maxSNP ) {
        ...
        for (j in 1:(dim(snp.pos)[1])) {
            ...
            hap.test <- msr.gaussian.haplotype.test.unadjusted ( geno,trait,  lim =lim,
                baseline.hap=baseline.hap , min.count=min.count )
...
} ## end of msr.gaussian.forward.unadjusted ###################################################
