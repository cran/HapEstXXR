...
function ( famid , patid , fid , mid , sex , trait , CHR , SNP , POS ,
  ...
  if ( !is.matrix(geno.matrix) ) geno.matrix <- as.matrix(geno.matrix)

  ## Achtung: Missing al -1 kodiert !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  geno.matrix <- alleleRto1 ( geno.matrix , miss.val=-1 ) # hier könnte Probl. entstehen,
  ...
  for ( i in 1:N )      {
      ...
      geno <- factor(geno,levels=c(-1,0,1,2),labels=c("0\t0","1\t1","2\t1","2\t2"))
      txt1 <- paste(  c(famid[i],patid[i],fid[i],mid[i],sex[i],trait[i])   , collapse="\t")
      txtgeno <- paste (  geno , collapse="\t" )
      txt <- paste ( txt1 , txtgeno , sep="\t" )
      cat (  txt , "\n" , sep="" , file=linkage.file , append=TRUE )
      if ( (i%%1000) == 0 ) cat ( i , "\n" )
  }
  ...
  for ( i in 1:ns ) {
    txtmap <- paste ( as.integer(CHR[i]) , as.character(SNP[i]) ,  as.integer(POS[i]) )
    cat (  txtmap , "\n" , sep="" , file=map.file , append=TRUE )
  }
  ...
  if ( !all(is.na(covariates)) ) {
    ...
    for ( i in 1:N ) {
      txt <- paste ( c(famid[i] , patid[i] , covariates[i,]) , collapse="\t" )
      cat (  txt , "\n" , sep="" , file=cov.file , append=TRUE )
      if ( (i%%1000) == 0 ) cat ( i , "\n" , sep="" )
    }
  }
}
