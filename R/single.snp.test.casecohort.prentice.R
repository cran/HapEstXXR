...
function ( snps, trait ,
 ...
 for ( i in 1:length(trait) ) {
  ...
  if ( robust==TRUE ) {
    res <-as.data.frame(matrix(0,nrow=nloci , ncol=11), stringsAsFactors = F)
  } else {
    res <-as.data.frame(matrix(0,nrow=nloci , ncol=10), stringsAsFactors = F)
  }
 ...
 if (!all(is.na(stratvar))) {
   ...
   for ( ii in 1:dim(stratvar)[2] ) {
    ...
    for ( j in 1:dim(xx)[2] ) {
      ...
      phfit <- summary(coxph( as.formula(fml) ,
          method=ties , robust=robust  ))  # marker and covariates
      ...
      res [j,] <-
           ...
           (phfit$coefficients)[1,c("Pr(>|z|)")] , LR  , Nagelkerke.R2)   ,format="fg"))  )
  ...
  for ( j in 1:dim(xx)[2] ) {
      ...
      x <- as.matrix(as.numeric(alleleRto1 ( xx[,j] )),ncol=1)
      phfit <- summary(coxph(  ysurv  ~  cbind ( x , adj.mat ) + cluster(as.factor(1:length(id))) ,
          method=ties, robust=robust  ))  # marker and covariates
  ...
  if ( robust==TRUE ) {
    colnames(res) <- c("SNP","N","type","beta","se(beta)", "exp(beta)", "lower.95" , "upper.95" ,"p.value", "LR" , "Nagelkerke.R2")
  } else {
    colnames(res) <- c("SNP","N","type","beta","robust se (beta)", "exp(beta)", "lower.95" , "upper.95","LR" ,"Nagelkerke.R2")
  }
