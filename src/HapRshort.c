...
/*********************************************************************/
/*                                                                   */
/*    HaptdpnR.c is version of haptdpnR.c for genome-wide search     */
...
double   *hap,            /* Prob. tats. vorkomm. Haplotypen                */
	...
	**pep,            /* probabilities of states_haptdpp for each family        */
...
int       first,          /* flaf for simulations                           */
	  ...
	  nf,             /* number of families in input file               */
         ...
         *nstate,         /* number of possible states_haptdpp per genotype         */
	 *nfstate,        /* number of states_haptdpp per family                    */
	 *mstate,         /* nu of possible states_haptdpp per genotype best state  */
...
uint   ***state,          /* list haplotypes pairs per genotype             */
       ***fstate,         /* list states_haptdpp of families                        */
...
void pspace_haptdpp(char *geline, int n)
{
...
void swap_i_haptdpp(int *u, int *v)
{
  int w = *u;   *u = *v;   *v = w;
}


void swap_ui_haptdpp(uint *u, uint *v)
{
  uint w = *u;   *u = *v;   *v = w;
}


void swap_d_haptdpp(double *u, double *v)
{
  double w = *u;   *u = *v;   *v = w;
}


bool repeating_haptdpp(int *d, int n)
{
  int i;
...
int comp_haptdpp(char *a, char *b)
...
uint *initPosition_haptdpp(uint *pos, uint n)
{
  uint i;
...
int drawHaplo_haptdpp(double *prob, double chance)
{
  int    i;
...
/*    prod *= mult;                                              */
    ...
    swap_ui_haptdpp(&fstate[n][maxi][0], &fstate[n][i][0]);
    swap_ui_haptdpp(&fstate[n][maxi][1], &fstate[n][i][1]);
    swap_ui_haptdpp(&fstate[n][maxi][2], &fstate[n][i][2]);
    swap_ui_haptdpp(&fstate[n][maxi][3], &fstate[n][i][3]);
    mstate[n] ++;
  }     /* end i-loop */
...
void rechap_haptdpp(int i, int k1, int k2)
{
...
double log_L_haptdpp(int *mg, double *max_prob)
{
  double x = 0.0;
  int    i;
...
void printHaplotype_haptdpp(int haplo, int len, char *hpres)
{
  int i;
  char hp[len];
...
void sortByProb_haptdpp(double *prob, int *geno, int n)
{
  int    i, j;

  for (i=1; i<n; i++)
    for (j=i; (j > 0) && (prob[j] > prob[j-1]); j--) {
      swap_d_haptdpp(&prob[j], &prob[j-1]);
      swap_i_haptdpp(&geno[j], &geno[j-1]);
    }
}



void states_haptdpp(int ii, ped* fam, char **gent)
{
  ...
  for(i=0;i<nfa;i++){
	...
	for(k3=0;k3<len-1;k3++){
     ...
     if( comp_haptdpp(gev,gent[fam[ii].c[itt]]) == 0 ) it1[itt] = 1;
...
/*        if(fstate[ii][i][0] != fstate[ii][i][1]) mult *= 2; 
	  ...
	  for(j=0;j<4;j++){
		  hapi = fill_haptdpp(fstate[ii][i][j]);
...
void printChild_haptdpp(int ii, int jj, int kkk, char **gent, int prin, char *hres)
{
  ...
  for(k1=0;k1<k1e;k1++)
    for(k2=0;k2<k2e;k2++){
	   ...
	   gev[len-1]='\0';
	   if( comp_haptdpp(gev,gent[fam[ii].c[jj]]) == 0 ){
		   kk[mult][0] = k1;
		   kk[mult][1] = k2;
		   mult++;
	   }
	   ...
	   if(mult == 1){
		   k1 = kk[0][0];
		   k2 = kk[0][1];
		   if(prin)printHaplotype_haptdpp(hf[k1],len,hres);
		   fdat[ii].chap[jj][0][0] = hf[k1];
		   fdat[ii].chap[jj][2][0] = k1 ? hf[0] : hf[1];
		   if(prin)strcat(hres," <> \0");
		   if(prin)printHaplotype_haptdpp(hm[k2],len,hres);
		   ...
		   if(prin){
		   strcat(hres," >> \0");
		   printHaplotype_haptdpp(fdat[ii].chap[jj][0][1],len,hres);
		   strcat(hres," <> \0");
		   printHaplotype_haptdpp(fdat[ii].chap[jj][1][1],len,hres);
		   }
                   }
}
...
void tdtpchi(int nfam, int*hcc, int *hl, int nhap, double **om, char **gent, char **dres)
{
  ...
  *
  for(i=0;i<nfstate[nfam];i++){
  printChild_haptdpp(nfam,j,i,gent,0,lino);
...
/*
double tdtmax_haptdpp(int nfam, int nhap, int *hcc, int *hl, int *imax, char *hmax, int length, char **gent, int sim, char **tres)
*/
double tdtmax_haptdpp(int nfam, int nhap, int *hcc, int *hl, int *imax, char *hmax, int length, char **gent, int sim)
{
  char      lino[200], lin[200], spa[20];
  int 	    i, j, js, k, ks, t, n, y, ifa, ifb, mult;
  double    s, smax, mt, mut, *sis, **tmat;
  void      printHaplotype_haptdpp(), pspace_haptdpp();
  void      printChild_haptdpp();
  ...
  for(j=0;j<nfam;j++)if(xsk[j]){             /* all families */
  ...
  */
  y=1 ; /*?*/
  for(js=0;js<nfstate[j];js++){    /* all states_haptdpp per family */
  printChild_haptdpp(j,k,js,gent,0,lino);
  mult = fdat[j].mult[k];
  for(ks=0;ks<mult;ks++){
	if( fdat[j].fhet ){  
	ifa=0;
	...
	if(ifa==1 && ifb==0){if(sim && y==1){
	  ...
	  for(i=0;i<nhap;i++){
	  printf("\n%4d ",hl[i]);
	  printHaplotype_haptdpp(hcc[hl[i]],length);
	  printf(" >> tr:%8.4f  ntr:%8.4f  s1: %8.4f  s: %8.4f\n",
	  tmat[0][i],tmat[1][i],sis[i],s);
          }
	  printf("\n%4d ",hl[nhap]);
          pspace_haptdpp(length-1);
	  printf(" >> tr:%8.4f  ntr:%8.4f  s1: %8.4f  s: %8.4f\n",
	  tmat[0][nhap],tmat[1][nhap],sis[nhap],s);
	  */
	  *hmax='\0';
	  printHaplotype_haptdpp(hcc[*imax],len,hmax);
	  /*
	  Rprintf("%s :::: %d\n",hmax,*imax);
	  */
	  ...
	  Rprintf("\n%4d ",hl[i]);
	  */
	  /* sprintf(lino,"%4d \0",hl[i]); 06.11.2014/SKn */
       sprintf(lino,"%4d %s",hl[i], CharNull);
    
	  printHaplotype_haptdpp(hcc[hl[i]],length,lino);
	  /*
	  Rprintf("%s",lino);
   ...
   sprintf(lin," >> tr:%8.4f  ntr:%8.4f  s1: %8.4f  s: %8.4f%s",
	  ...
	  */
	  ...
	  pspace_haptdpp(spa,length);

      /*  Rprintf("%s    ",spa);         rest of haplotypes collected */
	  strcpy(lino,spa);
	  strcat(lino,"    \0");
	  /*
	  Rprintf(" >> tr:%8.4f  ntr:%8.4f  s1: %8.4f  s: %8.4f\n\n",
	  ...
	  */
	  /* sprintf(lin," >> tr:%8.4f  ntr:%8.4f  s1: %8.4f  s: %8.4f\0",
			            tmat[0][nhap],tmat[1][nhap],sis[nhap],s); 06.11.2014/SKn */
        sprintf(lin," >> tr:%8.4f  ntr:%8.4f  s1: %8.4f  s: %8.4f%s",
...
/*
void haptdpn(char **famid, char **pid, char **gent, int *qtrait, int *xnp,
	     double *lim, char **likeres, char **freqres, char **hapres,
	     char **tdtres,char **pvres)
*/

void haptdpn(char **famid, char **pid, char **gent, int *qtrait, int *xnp,
	     double *lim, char **pvres)
{
  bool      loop;
  char  /*   *filename = InputFile,   name of the input file              */
            lino[400], lin[400];
  ...
  */
  ...
  iterations = 0;
    ...
    */
    ...
    for(i=0;i<ng;i++)rechap_haptdpp(i, 0, len-1);
    ...
    for(i=0;i<nf;i++)states_haptdpp(i ,fam,gent);

    ex = 0;
    for(i=0;i<nf;i++){
	    if( nfstate[i] == 0 ){
/*          Rprintf("\n Contradiction in family %s",fam[i].name);     */
	    xsk[i] = 0;
	    nall  -= 4;  /* skip this family comp_haptdppletely */
	    ex = 1;
	    }
    }
    /*
    ...
    */
    ...
    for(i=0;i<nf;i++)if(xsk[i])hapcount_haptdpp(i);
    ...
    for (i=0; i<nf; i++)if(xsk[i])selprob_haptdpp(i);


    likold = likea_haptdpp();

    /* Continue comp_haptdpputation of mean probabilities */
    ...
    do {
      ...
      /* Recomp_haptdppute mean probabilities */
      for (i=0; i<nh; i++) {
      ...
      if ( iterations == 1 ) {
        ...
        for (i=0; i<nh; i++)
          if ( hapnew[i] >= 0.01 ) {
            ...
            printHaplotype_haptdpp(hc[i], len);
            printf("\n");
          }
        printf("\n");
	*/
        ...
        printHaplotype_haptdpp(hc[fstate[i][0][0]],len);
        printf(" <> ");
        printHaplotype_haptdpp(hc[fstate[i][0][1]],len);
	
        printf("\n%s %s %s >> ",fam[i].name,fam[i].id[1],geno[fam[i].mo]);
        printHaplotype_haptdpp(hc[fstate[i][0][2]],len);
        printf(" <> ");
        printHaplotype_haptdpp(hc[fstate[i][0][3]],len);

	for(k=0;k<fam[i].nchi-1;k++){
        printf("\n%s %s %s >> ",fam[i].name,fam[i].id[k+2],gent[fam[i].c[k]]);
	printChild_haptdpp(i, k,0);
	}
        }
        printf("\n\n");
      }
    ...
    */
	...
	/* sprintf(lino,"%2d >> \0",i+1); 06.11.2014/SKn */
     ...
     printHaplotype_haptdpp(hc[i], len,lino);
        /* sprintf(lin," >> %8d >> %7.4f\0", hc[i], hapnew[i] );06.11.2014/SKn*/ /*, hap[i]);*/
           sprintf(lin," >> %8d >> %7.4f%s", hc[i], hapnew[i], CharNull); 
	strcat(lino,lin);
	/*
	...
	*/
	hlist[nhap++] = i;
      ...
      */
      ...
      /* start find best states_haptdpp after MLE   10.8.2000 ROHDE  */

      pimax  = imatrix(nf,20);               /* ROHDE  10.8.2000 */
      p2max = init_dvector(NULL,0.0,nf);     /* ROHDE  10.8.2000 */
      for(i=0;i<nf;i++)max_prob[i] = 0.0;    /* ROHDE  10.8.2000 */
      for (i=0;i<nf;i++)if(xsk[i]){
      for(j=0;j<20;j++)pimax[i][j] = -1;
      iqual=1;
...
*    printf("\n%3d  %s >> ",i+1,geno[fam[i].fa]);      *
      sprintf(lino,"%4s %4s %s >> \0",
		      fam[i].name,fam[i].id[0],geno[fam[i].fa]);
      printHaplotype_haptdpp(hc[fstate[i][pimax[i][0]][0]],len,lino);
      strcat(lino," <> \0");
      printHaplotype_haptdpp(hc[fstate[i][pimax[i][0]][1]],len,lino);
...
*    printf("\n%3d  %s >> ",i+1,geno[fam[i].mo]);      *
      ...
      printHaplotype_haptdpp(hc[fstate[i][pimax[i][0]][2]],len,lino);
      strcat(lino," <> \0");
      printHaplotype_haptdpp(hc[fstate[i][pimax[i][0]][3]],len,lino);
      *
      strcpy(hapres[jjx++],lino);
      /
...
*    printf("\n%3d  %s >> ",i+1,gent[fam[i].c[k]]);     *
      ...
      printChild_haptdpp(i, k,pimax[i][0],gent,1,lino);
      *
      Rprintf("%s\n",lino);
      *
      *
...
*****/
      ...
      /*
      Rprintf("\n Haplotypes after MLE 2\n");
      */

      /* endfind best states_haptdpp after MLE   10.3.2000 ROHDE  */
    ...
    /*
    smax = tdtmax_haptdpp(nf,nhap,hc,hlist,&imaxhap,lino,len,gent,0,tdtres);
    */
    smax = tdtmax_haptdpp(nf,nhap,hc,hlist,&imaxhap,lino,len,gent,0);
    /* sprintf(lino,"%g \0",smax); 06.11.2014/SKn */
       sprintf(lino,"%g %s",smax, CharNull);
    lin[0]='\0';
    printHaplotype_haptdpp(hc[imaxhap],len,lin);
    strcat(lino,lin);
    ...
    nmin = 0; itn=0; itp=0;
    for(i=0;i<10000;i++){
    ssim = tdtmax_haptdpp(rng,nf,nhap,hc,hlist,&imaxhap,len,gent,1,tdtres);
    if( ssim < smax ) nmin++;
    }
