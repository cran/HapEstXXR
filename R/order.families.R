...
function ( famid, patid, fid, mid , sex=NA ) {
  ...
  if ( !all(is.na(sex)) ){
     if ( ! all ( sex[!is.na(sex)] %in% c(0,1,2)) ) {
       stop ("Error in order.families: sex should be 1 (male), 2 (female) or 0 (unkown).")
     }
  ...
  if (any(duplicated(patid))) { stop("Error in order.families: All patid values must be unique.") }
  if (any(patid==0)) { stop("Error in order.families: All patid have to be not equal to 0.") }
  n <- length(patid)
  ...
  generation <- rep(0, n)
  for (i in 1:n) {
        child <- match(mid, patid[parents], nomatch = 0) + match(fid,
            patid[parents], nomatch = 0)
        if (!all(child == 0)) {
          if (i == n) {
              stop(paste("Error in order.families: Contradiction in pedigrees." ,sep=""))
          }
          parents <- (child > 0)
  ...
  if ( any(generation>1) ) {
    stop ( paste ( "Error in order.families: Only nuclear families are allowed.",sep="")  )
  }
