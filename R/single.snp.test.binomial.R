...
function ( snps, trait, adj.var=NULL , prt=T  ) {
  ...
  lower.95 <- exp(beta-xalpha*beta.stderr)
    res <- data.frame ( snp=1:ns , N=nind , type="binomial",
      beta=beta, "se(beta)"=beta.stderr, "OR"=exp(beta),
      lower.95=lower.95, upper.95=upper.95  , test="Chisq",
      p.value=pval,aic=aic , stringsAsFactors=F)
  colnames(res) <- c("SNP","N","type","beta","se(beta)", "exp(beta)", "lower.95" ,
